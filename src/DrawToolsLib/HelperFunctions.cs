...
namespace DrawToolsLib
{
    ...
    static class HelperFunctions
    {
        ...
        /// <summary>
        /// Determines whether the specified drawing canvas has selection.
        /// </summary>
        /// <param name="drawingCanvas">The drawing canvas.</param>
        /// <returns>
        /// 	<c>true</c> if the specified drawing canvas has selection; otherwise, <c>false</c>.
        /// </returns>
        public static bool HasSelection(DrawingCanvas drawingCanvas)
        {
            for (var i = 0; i < drawingCanvas.Count; i++)
            {
                if(drawingCanvas[i].IsSelected) return true;
            }
        ...
        public static bool CanApplyProperties(DrawingCanvas drawingCanvas)
        {
            foreach(GraphicsBase graphicsBase in drawingCanvas.GraphicsList)
            {
                ...
                if ( graphicsText == null )
                {
                    ...
                    if (graphicsBase.LineStyle != drawingCanvas.LineStyle)
                    {
                        return true;
                    }
                }
        ...
        public static void ApplyProperties(DrawingCanvas drawingCanvas)
        {
            ...
            if (ApplyLineStyle(drawingCanvas, drawingCanvas.LineStyle, false))
            {
                wasChange = true;
            }
